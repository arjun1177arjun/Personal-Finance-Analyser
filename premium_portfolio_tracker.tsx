...
const POPULAR_STOCKS = [
  { symbol: "RELIANCE", name: "Reliance Industries Ltd", price: null, prevClose: null },
  { symbol: "TCS", name: "Tata Consultancy Services Ltd", price: null, prevClose: null },
  ...
  { symbol: "HINDUNILVR", name: "Hindustan Unilever Ltd", price: null, prevClose: null },
  { symbol: "SBIN", name: "State Bank of India", price: null, prevClose: null },
  { symbol: "BHARTIARTL", name: "Bharti Airtel Ltd", price: null, prevClose: null },
  { symbol: "ITC", name: "ITC Limited", price: null, prevClose: null },
  { symbol: "TATAMOTORS", name: "Tata Motors Ltd", price: null, prevClose: null },
  { symbol: "WIPRO", name: "Wipro Limited", price: null, prevClose: null },
  { symbol: "RELIGARE", name: "Religare Enterprises Ltd", price: null, prevClose: null },
  { symbol: "RPOWER", name: "Reliance Power Ltd", price: null, prevClose: null },
  { symbol: "RELIANCE-INFRA", name: "Reliance Infrastructure", price: null, prevClose: null },
...
const INITIAL_TRANSACTIONS = [
  { id: 't1', profileId: 'p1', symbol: 'RELIANCE', name: 'Reliance Industries Ltd', type: 'BUY', price: 200.00, qty: 10, date: '2026-05-14' },
  { id: 't1-2', profileId: 'p1', symbol: 'RELIANCE', name: 'Reliance Industries Ltd', type: 'BUY', price: 220.00, qty: 5, date: '2026-05-18' },
  { id: 't2', profileId: 'p2', symbol: 'TATAMOTORS', name: 'Tata Motors Ltd', type: 'BUY', price: 580.00, qty: 15, date: '2026-04-10' },
  { id: 't3', profileId: 'p2', symbol: 'TCS', name: 'Tata Consultancy Services', type: 'BUY', price: 3400.00, qty: 5, date: '2026-02-14' }
];

const INITIAL_GTT = {
  'p1': {
    'RELIANCE': { type: 'SELL', price: 300.00 }
  },
  'p2': {
    'TATAMOTORS': { type: 'SELL', price: 680.00 }
  },
  'p3': {}
};
...
export default function App() {
  ...
  const addConsoleLog = (text) => {
    setConsoleLog(prev => [`[${new Date().toLocaleTimeString()}] ${text}`, ...prev.slice(0, 30)]);
  };
  ...
  const { holdings, realizedPnL, totalCostBasis, totalCurrentValue } = useMemo(() => {
    const profileTransactions = transactions.filter(t => t.profileId === activeProfileId);
    
    // Process chronologically (oldest first) to establish active lots & FIFO settlements
    const sorted = [...profileTransactions].sort((a, b) => new Date(a.date) - new Date(b.date));
    ...
    const calculatedHoldings = Object.keys(activeBuyLots).map(symbol => {
      const remainingLots = activeBuyLots[symbol].filter(l => l.qtyRemaining > 0);
      if (remainingLots.length === 0) return null;

      const totalRemainingQty = remainingLots.reduce((sum, l) => sum + l.qtyRemaining, 0);
      const totalCost = remainingLots.reduce((sum, l) => sum + (l.qtyRemaining * l.price), 0);
      const avgPrice = totalRemainingQty > 0 ? totalCost / totalRemainingQty : 0;

      const mStock = marketPrices.find(s => s.symbol === symbol) || { price: null, prevClose: null, name: symbol };
      ...
      const totalProfitLossPct = (hasPrice && totalCost > 0) ? (totalProfitLoss / totalCost) * 100 : null;

      const dayChangeAmtPerShare = hasPrice ? (mStock.price - mStock.prevClose) : null;
      const totalDayChangeAmt = (hasPrice && mStock.prevClose !== null) ? (totalRemainingQty * dayChangeAmtPerShare) : null;
      const dayChangePct = (hasPrice && mStock.prevClose) ? (dayChangeAmtPerShare / mStock.prevClose) * 100 : null;
    ...
    const costBasis = calculatedHoldings.reduce((sum, h) => sum + h.totalCost, 0);
    
    const hasAnyUnsynced = calculatedHoldings.some(h => h.currentPrice === null);
    const currValue = (!hasAnyUnsynced && calculatedHoldings.length > 0) 
  ...
  const overallPnL = totalCurrentValue !== null ? (totalCurrentValue - totalCostBasis) : null;
  const overallPnLPct = (totalCostBasis > 0 && overallPnL !== null) ? (overallPnL / totalCostBasis) * 100 : 0;

  const activeProfile = profiles.find(p => p.id === activeProfileId) || profiles[0];

  // Robust structured content generator utilizing Search-Grounded Gemini 2.5 API
  const fetchQuotesUsingGemini = async (symbols, retries = 5, delay = 1000) => {
    const apiKey = ""; // Injected automatically at runtime
    ...
    const promptText = `Find the exact real-time current market price and previous day's close price (in local native currencies, mainly INR for Indian NSE/BSE stocks, USD for US equities) for the following portfolio symbols: ${symbols.join(', ')}. Use your Google Search tool to fetch exact actual coordinates. Do NOT generate placeholders.`;
    ...
    const payload = {
      ...
      tools: [{
        google_search: {} // Enable real-time Google search grounding
      }],
      generationConfig: {
          ...
          properties: {
            quotes: {
              ...
              items: {
                ...
                properties: {
                  symbol: { type: "STRING" },
                  price: { type: "NUMBER", description: "Latest current stock trading price" },
                  prevClose: { type: "NUMBER", description: "Previous day close price" }
                },
                required: ["symbol", "price", "prevClose"]
              }
            }
          },
          required: ["quotes"]
    ...
    const makeRequest = async (attempt = 1) => {
      try {
        ...
        if (!response.ok) {
          ...
          console.log(`HTTP status: ${response.status} | responseText: ${errorResponseText}`);
          throw new Error(`HTTP Error Status ${response.status}`);
        }

        const data = await response.json();
        const outputText = data.candidates?.[0]?.content?.parts?.[0]?.text;
        if (!outputText) throw new Error("Received empty generated payload from Gemini model.");

        return JSON.parse(outputText);
      } catch (err) {
        if (attempt <= retries) {
          const nextDelay = delay * Math.pow(2, attempt - 1);
          addConsoleLog(`Network constraint. Re-routing Gemini request in ${(nextDelay / 1000)}s...`);
  ...
  const handleManualRefresh = async () => {
    ...
    try {
      addConsoleLog(`Constructing grounded search prompt for: ${symbolsToQuery.join(', ')}`);
      
      const parsedOutput = await fetchQuotesUsingGemini(symbolsToQuery);
      ...
      const failedSymbols = symbolsToQuery.filter(sym => !returnedSymbols.has(sym.toUpperCase()));
      ...
      const updatedMarketList = marketPrices.map(oldStock => {
        ...
        const freshQuote = results.find(q => q.symbol.toUpperCase() === uppercaseSymbol);
      ...
      const currentTimeString = new Date().toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
  ...
  const selectStockFromSearch = async (stock) => {
    ...
    const alreadyExists = marketPrices.some(s => s.symbol.toUpperCase() === uppercaseSymbol);
    let updatedPrices = [...marketPrices];
    let matchedStock = marketPrices.find(s => s.symbol.toUpperCase() === uppercaseSymbol);
    ...
    try {
      addConsoleLog(`Fetching immediate quote for selected stock: ${stock.symbol}`);
      const singleQuoteOutput = await fetchQuotesUsingGemini([stock.symbol]);
      ...
      if (freshQuote) {
        ...
        setMarketPrices(prev => prev.map(s => {
  ...
  const handleAddStock = (e) => {
    e.preventDefault();
    if (!selectedStock) {
      triggerNotification("Please select a stock from the dropdown suggestions.", "error");
      return;
    }
    ...
    if (txType === 'SELL') {
      const activeHold = holdings.find(h => h.symbol === selectedStock.symbol);
      const activeQtyAvailable = activeHold ? activeHold.qty : 0;
      if (qty > activeQtyAvailable) {
        triggerNotification(`Insufficient holding. You only own ${activeQtyAvailable} shares of ${selectedStock.symbol}.`, "error");
        return;
      }
    }
    ...
    triggerNotification(`Recorded ${txType} transaction for ${qty} shares of ${newTx.symbol}`);
  };

  // Delete profile
  const handleDeleteProfile = () => {
    if (profiles.length <= 1) {
      triggerNotification("You must maintain at least one active profile.", "error");
      return;
    }
  ...
  const handleSaveEditStock = (e) => {
    ...
    let otherTxs = transactions.filter(t => t.profileId !== activeProfileId || t.symbol !== editingStockSymbol);
    
    const updatedBuyTxs = editingLots.map(lot => ({
      id: lot.id,
      profileId: activeProfileId,
      symbol: editingStockSymbol,
      name: marketPrices.find(s => s.symbol === editingStockSymbol)?.name || editingStockSymbol,
      type: 'BUY',
    ...
    if (editGttEnabled && editGttPrice) {
      setGttOrders(prev => ({
        ...
        [activeProfileId]: {
          ...prev[activeProfileId] || {},
          [editingStockSymbol]: { type: editGttType, price: parseFloat(editGttPrice) }
        }
      }));
    } else {
      setGttOrders(prev => {
    ...
    triggerNotification(`Successfully adjusted parameters for ${editingStockSymbol}`);
  };

  // Remove entire portfolio transactions for a stock
  const handleRemoveStock = (symbol) => {
    const cleanList = transactions.filter(t => t.profileId !== activeProfileId || t.symbol !== symbol);
  ...
  const handleExportCSV = () => {
    const profileTxs = transactions.filter(t => t.profileId === activeProfileId);
    ...
    const headers = ["Transaction ID", "Date", "Symbol", "Asset Name", "Action Type", "Price (INR)", "Quantity", "Net Total (INR)"];
    ...
    <div className="min-h-screen bg-[#0b0f19] text-[#e2e8f0] font-sans antialiased p-4 sm:p-6 lg:p-8 flex flex-col justify-between">
      ...
      <div className="max-w-7xl mx-auto w-full space-y-6">
        ...
        <div className="flex items-center justify-between gap-3 bg-[#111622] p-4 rounded-xl border border-[#1e293b] overflow-x-auto whitespace-nowrap scrollbar-thin">
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            ...
            <div className="flex items-center gap-2 bg-[#1c2436] px-2.5 py-1.5 sm:px-3.5 rounded-full border border-[#334155] shrink-0">
              ...
              <span className="text-xs sm:text-sm font-bold text-white">{activeProfile.name}</span>
            </div>
          </div>
          ...
          {lastSyncedTime && (
            <div className="hidden md:flex items-center gap-2 text-xs text-slate-400 font-mono">
              ...
              Last synced market prices: <span className="text-emerald-400 font-bold">{lastSyncedTime}</span>
            </div>
          )}
        ...
        {apiError && (
          <div className="bg-rose-950/20 border border-rose-900/50 rounded-xl p-4 text-xs text-rose-400 space-y-1">
            <p className="font-bold">⚠️ Market Quote Synchronization Interrupted</p>
            <p className="font-mono text-[11px] opacity-85">Details: {apiError}</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-[#111622] p-5 rounded-xl border border-[#1e293b]">
            <p className="text-[11px] font-bold tracking-wider text-slate-500 uppercase">Holdings</p>
            <p className="text-3xl font-black text-white mt-1.5">{holdings.length}</p>
          </div>

          {/* INVESTED */}
          <div className="bg-[#111622] p-5 rounded-xl border border-[#1e293b]">
            <p className="text-[11px] font-bold tracking-wider text-slate-500 uppercase">Invested</p>
          ...
          {/* CURRENT VALUE */}
          <div className="bg-[#111622] p-5 rounded-xl border border-[#1e293b]">
            <p className="text-[11px] font-bold tracking-wider text-slate-500 uppercase">Current Value</p>
            <p className="text-2xl font-mono font-black text-white mt-1.5">
              {lastSyncedTime && totalCurrentValue !== null ? formatINR(totalCurrentValue) : "Not synced yet"}
            </p>
          </div>

          {/* TOTAL P&L */}
          <div className="bg-[#111622] p-5 rounded-xl border border-[#1e293b] relative overflow-hidden">
            <p className="text-[11px] font-bold tracking-wider text-slate-500 uppercase">Total P&amp;L</p>
            <div className="mt-1.5 flex items-baseline gap-2">
              {lastSyncedTime && overallPnL !== null ? (
                <>
                  <span className={`text-2xl font-mono font-bold ${overallPnL >= 0 ? 'text-[#22c55e]' : 'text-[#ef4444]'}`}>
                    {overallPnL >= 0 ? `+` : ``}{overallPnL.toFixed(2)}
                  </span>
                  <span className={`text-xs font-semibold ${overallPnL >= 0 ? 'text-[#22c55e]' : 'text-[#ef4444]'}`}>
                    ({overallPnLPct.toFixed(2)}%)
                  </span>
                </>
              ) : (
                <span className="text-sm font-semibold text-slate-400">Not synced yet</span>
              )}
            </div>
            {/* Horizontal status accent bar */}
            {lastSyncedTime && overallPnL !== null && (
              <div className={`absolute bottom-0 left-0 right-0 h-1 ${overallPnL >= 0 ? 'bg-[#22c55e]' : 'bg-[#ef4444]'}`} />
            )}
          </div>
        ...
        {showConsole && (
          <div className="bg-[#0e1320] border border-[#1e293b] rounded-xl p-4 font-mono text-[11px] text-slate-400 space-y-2 max-h-40 overflow-y-auto">
            <div className="flex justify-between items-center text-xs text-slate-500 border-b border-[#1e293b] pb-1.5">
              ...
              {lastSyncedTime ? (
                <span className="text-[10px] text-slate-400">Last synced market prices: {lastSyncedTime}</span>
              ) : (
        ...
        <div className="space-y-4">
          ...
          {holdings.length === 0 ? (
            <div className="bg-[#111622] p-12 text-center rounded-xl border border-[#1e293b] space-y-4">
              ...
              <h3 className="text-lg font-bold text-white">No active assets tracked</h3>
              <p className="text-xs text-slate-400 max-w-sm mx-auto">Click "+ Add stock" above to log custom purchase details, target prices, and live simulated updates.</p>
              <button
            ...
            holdings.map(stock => {
              ...
              return (
                <div key={stock.symbol} className="bg-[#111622] rounded-xl border border-[#1e293b] overflow-hidden">
                  ...
                  <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-6 text-sm divide-y md:divide-y-0 md:divide-x divide-[#1e293b]">
                    ...
                    <div className="space-y-3 pt-0">
                      <p className="text-[10px] font-bold tracking-widest text-slate-500 uppercase">Purchase Details</p>
                      
                      <div className="space-y-2.5 max-h-40 overflow-y-auto scrollbar-thin pr-1">
                        {stock.purchases.map((lot, index) => (
                          <div key={lot.id || index} className="p-2 bg-[#171e2c] rounded border border-[#232e42] text-xs space-y-1">
                            <div className="flex justify-between">
                              <span className="text-[10px] font-bold text-[#22c55e]">LOT #{index + 1}</span>
                              <span className="text-slate-400 font-mono text-[10px]">
                                {new Date(lot.date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                              </span>
                            </div>
                            <div className="flex justify-between font-mono text-slate-300">
                              <span>Price:</span>
                              <span className="text-white font-semibold">{formatINR(lot.price)}</span>
                            </div>
                            <div className="flex justify-between font-mono text-slate-300">
                              <span>Shares remaining:</span>
                              <span className="text-white font-semibold">{lot.qtyRemaining} <span className="text-[10px] text-slate-500">/ {lot.qtyOriginal}</span></span>
                            </div>
                            <div className="flex justify-between border-t border-[#232e42] pt-1 font-mono text-[10px] text-slate-400">
                              ...
                              <span>{formatINR(lot.qtyRemaining * lot.price)}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="space-y-3 pt-4 md:pt-0 md:pl-6">
                      <p className="text-[10px] font-bold tracking-widest text-slate-500 uppercase">Market Data</p>
                      <div className="space-y-1.5 text-xs">
                        <div className="flex justify-between md:block">
                          <span className="text-slate-500 mr-2">Current price:</span>
                          <span className="font-mono font-bold text-white text-sm block md:inline">
                            {hasMarketSynced ? formatINR(stock.currentPrice) : "Not synced yet"}
                          </span>
                        </div>
                        <div className="flex justify-between md:block">
                          <span className="text-slate-500 mr-2">Total Returns:</span>
                          {hasMarketSynced && stock.totalProfitLoss !== null ? (
                            <span className={`font-mono font-semibold block md:inline ${isProfit ? 'text-[#22c55e]' : 'text-[#ef4444]'}`}>
                              {isProfit ? '+' : ''}{stock.totalProfitLoss.toFixed(2)} ({stock.totalProfitLossPct.toFixed(2)}%)
                            </span>
                          ) : (
                            <span className="text-slate-400 font-medium block md:inline">Not synced yet</span>
                          )}
                        </div>
                        <div className="flex justify-between md:block">
                          <span className="text-slate-500 mr-2">Today's change:</span>
                          {hasMarketSynced && stock.dayChangePct !== null ? (
                            <span className={`font-mono font-semibold block md:inline ${stock.dayChangePct >= 0 ? 'text-[#22c55e]' : 'text-[#ef4444]'}`}>
                              {stock.dayChangePct >= 0 ? '▲' : '▼'} {stock.dayChangePct.toFixed(2)}%
                            </span>
                          ) : (
                            <span className="text-slate-400 font-medium block md:inline">—</span>
                          )}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3 pt-4 md:pt-0 md:pl-6">
                      <p className="text-[10px] font-bold tracking-widest text-slate-500 uppercase">GTT Order</p>
                      
                      {gtt ? (
                        <div className="space-y-1.5 text-xs">
                          <div className="flex justify-between md:block">
                            <span className="text-slate-500 mr-2">Type:</span>
                            <span className="font-bold text-amber-500 uppercase tracking-wide block md:inline">{gtt.type}</span>
                          </div>
                          <div className="flex justify-between md:block">
                            <span className="text-slate-500 mr-2">Trigger price:</span>
                            <span className="font-mono font-bold text-white block md:inline">{formatINR(gtt.price)}</span>
                          </div>
                          <div className="flex justify-between md:block">
                            <span className="text-slate-500 mr-2">vs current:</span>
                            <span className="font-mono text-slate-300 font-semibold block md:inline">{gttDistanceStr}</span>
                          </div>
                        </div>
                      ) : (
                        <div className="py-1">
                          <p className="text-sm font-black text-rose-500 font-mono tracking-widest uppercase">NA</p>
                          <p className="text-[10px] text-slate-500 mt-1">No limit sell trigger active</p>
                        </div>
                      )}
                    </div>
        ...
        <div className="bg-[#111622] rounded-xl border border-[#1e293b] p-5">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-4">
            <div>
              ...
              <p className="text-[11px] text-slate-500">Chronological transaction journal and realized gain audits for {activeProfile.name}</p>
            </div>
            <div className="flex items-center gap-3 shrink-0">
              ...
              <div className="bg-[#161d2d] border border-[#334155] px-3 py-1.5 rounded-lg text-xs font-semibold">
                Lifetime Realized P&amp;L: <span className={`font-bold ${realizedPnL >= 0 ? 'text-[#22c55e]' : 'text-[#ef4444]'}`}>{formatINR(realizedPnL)}</span>
              </div>
            </div>
          </div>

          {transactions.filter(t => t.profileId === activeProfileId).length === 0 ? (
            <p className="text-xs text-slate-500 py-4 text-center">No transaction history entries logged for this profile.</p>
          ) : (
              ...
              <table className="w-full text-left text-xs text-slate-300">
                <thead>
                  <tr className="border-b border-[#1e293b] text-slate-500">
                    ...
                    <th className="py-2.5 font-bold text-right">Price per Share</th>
                  ...
                  {transactions
                    ...
                    .map(tx => (
                      <tr key={tx.id} className="hover:bg-[#161d2c]/40">
                        <td className="py-2.5 text-slate-400 font-mono">{tx.date}</td>
                        <td className="py-2.5 font-semibold text-white">{tx.symbol}</td>
                        <td className="py-2.5">
                          <span className={`font-bold px-1.5 py-0.5 rounded text-[10px] ${tx.type === 'BUY' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>
                            {tx.type}
                          </span>
                        </td>
                        <td className="py-2.5 text-right font-mono font-medium">{tx.qty}</td>
                        <td className="py-2.5 text-right font-mono text-slate-300">{formatINR(tx.price)}</td>
                        <td className="py-2.5 text-right font-mono text-white font-semibold">{formatINR(tx.qty * tx.price)}</td>
                      </tr>
                    ))}
                </tbody>
              </table>
        ...
        <div className="fixed inset-0 z-50 bg-[#07090f]/90 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#111622] border border-[#1e293b] rounded-xl max-w-lg w-full p-6 shadow-2xl space-y-4">
            <div className="flex justify-between items-center pb-2 border-b border-[#1e293b]">
              <h3 className="text-base font-bold text-white">Record Stock Transaction</h3>
              <button onClick={() => {
                setShowAddModal(false);
                setSearchQuery('');
                setSelectedStock(null);
            ...
            <form onSubmit={handleAddStock} className="space-y-4">
              ...
              <div className="relative" ref={autocompleteRef}>
                <label className="text-xs text-slate-400 font-semibold block mb-1">Search Symbol, ETF or Mutual Fund (e.g. TATAGOLD, INFY, TSLA)</label>
                <input 
                  type="text"
                  ...
                  <div className="absolute z-30 left-0 right-0 mt-1 bg-[#161d2d] border border-[#334155] rounded-lg max-h-64 overflow-y-auto shadow-2xl divide-y divide-[#1e293b]">
                    {searchLoading ? (
                      ...
                      searchResults.map(stock => (
                        ...
                        >
                          <div>
                            <span className="font-bold text-white text-xs block">{stock.symbol}</span>
                            <span className="text-[10px] text-slate-400 block truncate max-w-[280px]">{stock.name}</span>
                          </div>
                      ...
                      <div className="p-4 text-xs text-slate-400 text-center space-y-2">
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            const customSymbol = searchQuery.toUpperCase().trim();
                            const customStock = { symbol: customSymbol, name: customSymbol, price: null, prevClose: null };
                            setSelectedStock(customStock);
                            setMarketPrices([...marketPrices, customStock]);
                            setSearchQuery(`${customSymbol} (${customSymbol})`);
                            setShowSuggestions(false);
                            addConsoleLog(`Force registered custom lookup symbol: ${customSymbol}`);
                          }}
                      ...
                      <div className="p-3 text-xs text-slate-500 text-center">Type 2 or more characters to search...</div>
                    )}
                  </div>
                )}
              </div>
              ...
              {selectedStock && (
                <p className="text-xs text-emerald-400">
                  ✓ Selected: <strong>{selectedStock.name} ({selectedStock.symbol})</strong> • Last synced price: {selectedStock.price !== null ? formatINR(selectedStock.price) : "Not synced yet"}
                </p>
              )}

              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="text-xs text-slate-400 block mb-1">Shares Qty</label>
                  <input 
                    type="number"
                    required
                    placeholder="10"
              ...
              {txType === 'BUY' && (
                <div className="p-3 bg-[#161d2d] rounded-lg border border-[#334155]/60 space-y-2.5">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-xs font-semibold text-slate-200">Include Selling GTT Trigger Target</span>
                  </label>
                  ...
                  {gttEnabled && (
                    <div className="grid grid-cols-2 gap-2 pt-1 animate-fadeIn">
                      ...
                      <div>
                        <label className="text-[10px] text-slate-400 block mb-0.5">Trigger Price (₹)</label>
                        <input 
                          type="number"
                          step="0.01"
                          placeholder="300"
      ...
      {showEditModal && (
        <div className="fixed inset-0 z-50 bg-[#07090f]/90 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#111622] border border-[#1e293b] rounded-xl max-w-lg w-full p-6 shadow-2xl space-y-4">
            <div className="flex justify-between items-center pb-2 border-b border-[#1e293b]">
              <div>
                <h3 className="text-base font-bold text-white">Edit {editingStockSymbol} Parameters</h3>
                <p className="text-[11px] text-slate-400">Modify buy lots details or update targets</p>
              </div>
            ...
            <form onSubmit={handleSaveEditStock} className="space-y-4">
              ...
              <div className="space-y-3">
                <p className="text-xs font-bold text-slate-400">Individual Buy Lots</p>
                <div className="max-h-48 overflow-y-auto space-y-3 pr-1 scrollbar-thin">
                  {editingLots.map((lot, idx) => (
                    <div key={lot.id || idx} className="p-3 bg-[#172030] rounded-lg border border-[#232e42] space-y-2">
                      <p className="text-[10px] font-bold text-[#22c55e]">LOT #{idx + 1}</p>
                      <div className="grid grid-cols-3 gap-2">
                        <div>
                          <label className="text-[10px] text-slate-400 block mb-0.5">Shares</label>
                          <input 
                            type="number"
                            required
                        ...
                        <div>
                          <label className="text-[10px] text-slate-400 block mb-0.5">Price per Share</label>
                          <input 
                            type="number"
                            step="0.01"
                            required
              ...
              <div className="p-3 bg-[#161d2d] rounded-lg border border-[#334155] space-y-2.5">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input 
                    ...
                    className="accent-[#22c55e]"
                  />
                  <span className="text-xs font-semibold text-slate-200">GTT Trigger Target Active</span>
                </label>
                ...
                {editGttEnabled && (
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    ...
                    <div>
                      <label className="text-[10px] text-slate-400 block mb-0.5">Trigger Target Price (₹)</label>
                      <input 
                        type="number"
                        step="0.01"
                        value={editGttPrice}
      ...
      {showProfileSwitcher && (
        <div className="fixed inset-0 z-50 bg-[#07090f]/90 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#111622] border border-[#1e293b] rounded-xl max-w-sm w-full p-6 shadow-2xl space-y-4">
            <div className="flex justify-between items-center pb-2 border-b border-[#1e293b]">
              <h3 className="text-base font-bold text-white">Select active profile</h3>
              <button onClick={() => setShowProfileSwitcher(false)} className="text-slate-400 hover:text-white text-lg">✕</button>
            </div>
            ...
            <div className="space-y-2">
              {profiles.map(p => (
                ...
                >
                  ...
                  {p.id === activeProfileId && <span className="text-xs text-emerald-400">Active</span>}
                </button>
              ))}
            </div>
      ...
      {showCreateProfileModal && (
        <div className="fixed inset-0 z-50 bg-[#07090f]/90 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#111622] border border-[#1e293b] rounded-xl max-w-sm w-full p-6 shadow-2xl space-y-4">
            <div className="flex justify-between items-center pb-2 border-b border-[#1e293b]">
              <h3 className="text-base font-bold text-white">Create portfolio profile</h3>
              <button onClick={() => setShowCreateProfileModal(false)} className="text-slate-400 hover:text-white text-lg">✕</button>
            </div>

            <form onSubmit={handleCreateProfile} className="space-y-4">
              <div>
                <label className="text-xs text-slate-400 block mb-1">Holder name</label>
                <input 
                  type="text"
                  required
      ...
      <footer className="text-center text-[11px] text-slate-600 mt-6">
        <p>© 2026 folioX Finance Engine. Authenticated NSE/BSE &amp; NYSE quotes synced on demand.</p>
      </footer>
